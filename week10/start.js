...
function start() {
 ...
 // Load objects
  ...
  function drawModel(objectNumber) {
    ...
    if (objectNumber == 2) {
      let sub=glm.vec3(0, 0, 0);
      console.log("centterArray: "+centerArray[objectNumber].x+" "+centerArray[objectNumber].y+" "+centerArray[objectNumber].z);
      sub.x=cameraPos.x-centerArray[objectNumber].x  
      sub.y=cameraPos.y-centerArray[objectNumber].y 
      sub.z=cameraPos.z-centerArray[objectNumber].z  

      
      distanceArray[objectNumber]=glm.length(sub);
      // console.log("distance: "+objectNumber+" " +distanceArray[objectNumber]);
      
      

      if (distanceArray[objectNumber] >0 && distanceArray[objectNumber] < 5) {
        gl.drawArrays(gl.TRIANGLES, 0, 657408);
      }
      if (distanceArray[objectNumber] > 5 && distanceArray[objectNumber] < 7) {
        gl.drawArrays(gl.TRIANGLES, 657408, 131481);
      }
      if (distanceArray[objectNumber] > 7 && distanceArray[objectNumber] < 9) {
        gl.drawArrays(gl.TRIANGLES, 788889, 13143);
      }
      if (distanceArray[objectNumber] > 9 ) {
        gl.drawArrays(gl.TRIANGLES, 802032, 3285);
      }
      return;
    }
    ...
    if (objectNumber == 3) {
      ...
      if (distanceArray[objectNumber] > 0 && distanceArray[objectNumber] < 5) {  //LVL 0
        gl.drawArrays(gl.TRIANGLES, 0, 5127);
      }
      if (distanceArray[objectNumber] > 5 && distanceArray[objectNumber] < 7) { //LVL 1
        gl.drawArrays(gl.TRIANGLES, 5127, 1023);
      }
      if (distanceArray[objectNumber] > 7 && distanceArray[objectNumber] < 9) { //LVL 2
        gl.drawArrays(gl.TRIANGLES, 6150, 117);
      }
      if (distanceArray[objectNumber] > 9 ) {  //LVL 3
        gl.drawArrays(gl.TRIANGLES, 6267, 81);
      }
      return;
    }
